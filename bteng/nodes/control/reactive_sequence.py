...
def _is_condition_guard(node: "TreeNode") -> bool:
    ...
    guard, and a ``<Fallback>`` of conditions is an ordinary disjunction; both
...
class ReactiveSequenceNode(ControlNode):
    """Re-evaluates every guarding condition from the first, on every tick.
    ...
    * All SUCCESS → return SUCCESS.
    ...
    a CONDITION, or a wrapper (Inverter, a Fallback of conditions, …) whose
    ...
    What the dirty flag optimises is only the *action* children that already
    returned SUCCESS earlier in the sequence: re-entering a running or
    completed action is expensive and often not idempotent, so while an action
    child is RUNNING this node subscribes to every Blackboard reachable from
    its subtree and skips those preceding actions until some key is written.
    A blackboard write sets the dirty flag and the next tick re-evaluates the
    whole sequence from the first child.

    (Previously the fast path skipped conditions too, so a condition backed by
    anything other than a blackboard was never re-evaluated while a sibling
    action ran — and merely giving a child a Blackboard silently switched the
    node between reactive and non-reactive semantics.)
    """
    ...
    def _fast_tick(self) -> NodeStatus:
        ...
        # The reactive guarantee: conditions guarding the running action are
        # re-evaluated even when no blackboard key changed.
        for i in range(idx):
        ...
        # SUCCESS — continue evaluating remaining children
